...
This chapter first introduces some of the physics that understanding of the subsequent chapters rely on. Various atomic and charged particle mechanisms that generate electromagnetic radiation are described because these are the fundamental processes responsible for the generation of the photons that instruments observe and also serve as an important heat transport mechanism. Additionally, the conditions required for thermodynamic equilibrium are discussed because some regimes of the sun obey the corresponding mathematical descriptions while others do not. It turns out that non-local thermodynamic equilibrium processes do not strongly influence the results of later chapters, but it would be negligent not to acknowledge the inherent assumptions made and the care that must be taken. 

Section \ref{sec:tour} provides a brief tour of the sun from its center to the heliosphere encompassing the solar system. The basic physics previously outlined underlies much of the discussion here. Section \ref{sec:physicssolareruptiveevents} delves a little deeper into the physics of solar eruptive events. Section \ref{sec:spaceweather} briefly discusses the practical implications of solar eruptive events, namely space weather and its detrimental influences on human technology. Finally, Section \ref{sec:instruments} provides descriptions of the instruments that are critical components of the analyses in later chapters. 

\section{Relevant Basic Physics}
...
\paragraph{Spectral band definitions}
This dissertation will refer frequently to different bands of light by common names. The International Organization for Standardization (ISO) has defined specific values for these ranges, and the relevant subset of them are included in Table \ref{tab:spectraldefinitions}. 

\begin{table}[!h]
    \caption[Spectral band definitions]{
        Selected formal spectral band definitions adapted from ISO document 21348. 
    }
    \begin{center}
    \begin{tabular}{|l|c|} \hline
	Common name & Wavelength range (\AA) \\ \hline \hline
	Hard X-rays (HXRs) & $0.01 \leq \lambda < 1.0$ \\ \hline
	Soft X-rays (SXRs) & $1.0 \leq \lambda < 100$  \\ \hline
	Extreme Ultraviolet (EUV) & $100 \leq \lambda < 1210$ \\ \hline
	Visible & $3800 \leq \lambda < 7600$ \\ \hline
	Ultra High Frequency (UHF) & $1.00 \times 10^9 \leq \lambda < 1.00 \times 10^10$ \\ \hline
	\end{tabular}
    \\ \rule{0mm}{5mm}
    \end{center}
    \label{tab:spectraldefinitions}
\end{table}
...
Planck's equation describes blackbody emission: 
...
\noindent where $S$ is the spectral radiance of a body at a particular temperature, $\lambda$ is wavelength, $h$ is Planck's constant, $c$ is the speed of light, and other terms are as previously defined. This equation can be interpreted simply as a lower temperature resulting in lower energy emission (i.e., longer wavelength) and lower intensity (see Figure \ref{fig:blackbody}).

\begin{figure}[!h]
    \begin{center}
	    \includegraphics[width=100mm]{Images/PlankBlackBody.png}
    \end{center}
    \caption[Blackbody emission]{
        Blackbody emission at several different temperatures (blue, green, and red curves). The black curve shows the 
        Rayleigh-Jeans approximation to Plank's law and can be ignored here. Figure courtesy of 
        \href{https://commons.wikimedia.org/wiki/File:Black_body.svg}{Darth Kule}.
    }
    \label{fig:blackbody}
\end{figure} 
...
\begin{figure}[!h]
    ...
    \caption[Solar plasma $\beta$ versus height]{
        Model of solar plasma $\beta$ versus height from the photosphere through the corona in an active region. 
        The shaded region represents the open and closed field lines originating between a sunspot of 2500 G and a plage
        region of 150 G. (The plage curve can also represent older, decaying active regions that have no umbral 
        features.) Various data indicate that $\beta$ approaches unity at relatively low heights in the mid-corona. 
        Figure courtesy of \citet{Gary2001}.         
    }
    \label{fig:betavsheight}
\end{figure}

\subsection{Core}
The gravitational pressure and density in the core of stars is sufficient to ignite nuclear fusion. In a main sequence star at the midpoint of its life, like the sun, the primary fusion reaction is the conversion of hydrogen to helium. The majority of the sun is made of hydrogen (see Figure \ref{fig:sunabundance}) -- a reflection of its relative abundance in the universe at large. Fusion in the core of stars is responsible for producing elements up to iron; the fusion process for elements heavier than iron is endothermic and thus cannot be used by the star to support itself against gravity. Instead, heavier elements are produced during supernovae. Supernovae also spread the source star's fusion products far away where they are incorporated into newly forming stars. Thus, the sun has observable metals\footnote{``metals" here is in the astrophysical sense of all elements heavier than helium} such as Fe even though it has not reached the point in its life where it produces them itself. The metals in these second and third generation stars are not confined to the core; rather, they can be found even in the corona. In subsequent sections it will become clear that having various elemental species at different stages of ionization in the directly observable solar atmosphere provides a means of determining temperature and structure. The core itself -- in fact, everything below the photosphere -- is not directly observable. However, the field of helioseismology has developed methods for inferring many properties of the solar interior. 

Helioseismology takes advantage of the fact that the roiling ``bubbles" at the photosphere are the result of mass fluid-like motion through the entire sun -- oscillations due to resonant sound waves. By measuring Doppler shifts in the light, it is possible to derive velocities of the plasma and get an indication of interior plasma properties such as hydrogen abundance in the core \citep{Gough1990}, the depth of the convection zone \citep{Christensen-Dalsgaard1991}, helium abundance in the convection zone \citep{Basu1995}, the age of the sun \citep{Guenther1989}, and more. Additionally, models of stars are critical to our understanding of the solar interior and entire textbooks have been written on the subject (e.g., \citealt{Chandrasekhar1939, Kippenhahn2012}). Further details are beyond the scope of this dissertation, so we continue our tour of the sun and turn to the radiative zone. 
...
\begin{figure}[!h]
    ...
    \caption[AIA bandpasses]{
	    ...
	    Some emission lines are labeled with their corresponding ion. The cooler corona lines, such as Fe IX through 
	    Fe XII, are the better ones for coronal dimming analysis. 
	}
